...
//	public Timeout globalTimeout = new Timeout(500L, TimeUnit.MILLISECONDS);
	...
	@Test
	public void testFindReachableSetZero() {
		final Searcher<Cell> s = new Searcher<Cell>(maze);
		Set<Cell> reachableZero = new HashSet<Cell>();
		reachableZero.add(new Cell(1, 0));
		assertEquals(reachableZero,s.findReachableSet(0));
	}
	
	@Test
	public void testFindReachableSetOne() {
		final Searcher<Cell> s = new Searcher<Cell>(maze);
		Set<Cell> reachableZero = new HashSet<Cell>();
		reachableZero.add(new Cell(1, 0));
		reachableZero.add(new Cell(0, 0));
		reachableZero.add(new Cell(1, 1));
		reachableZero.add(new Cell(2, 0));
		assertEquals(reachableZero,s.findReachableSet(1));
	}
}
